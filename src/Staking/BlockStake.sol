...
contract BlockStake is Ownable {
  ...
  event Staked(address sender, uint256 amount, uint256 rewards, uint256 blockNumber);
  event Withdrawed(address sender, uint256 amount, uint256 rewards, uint256 blockNumber);
  ...
  constructor(address stakeToken_, address rewardToken_, uint256 rewardPerBlock_) Ownable(msg.sender) {
  ...
  function stake(uint256 amount_) external {
    ...
    debtAmountOfUsers[user] = (stakedAmountOfUsers[user] *  accumulatedRewardPerToken) / 1e18;
  ...
  function _transferReward(address user_) internal returns(uint256 reward) {
    ...
    reward = (stakedAmount * accumulatedRewardPerToken) / 1e18 - debtAmountOfUsers[user_];
  ...
  function _updateAccumulatedReward() internal {
    ...
    accumulatedRewardPerToken += (totalAccumulatedRewards * 1e18) / totalStakedAmount;
    lastUpdatedBlock = block.number;
  }
}